  ...
  SearchableOptionList.prototype = {
    ...
    defaults: {
      ...
      texts: {
        ...
        quickDelete: '&times;',
        searchplaceholder: 'Select', // Click here to search
      ...
      events: {
        ...
        onScroll: function () {
          ...
          var $scrollTarget = this.config.scrollTarget();
          
          this.config.maxHeight = Math.max($scrollTarget.innerHeight() / 3, 50);
          ...
          var selectionContainerYPos,
              ...
              inputOffsetBottom = this.$input.offset().top - $scrollTarget.offset().top
                  + this.$input.outerHeight(false),
              
              displayContainerAboveInput = this.config.displayContainerDirection === 'top' ||
                  (this.config.displayContainerDirection === 'auto' &&
                      (inputOffsetBottom > $scrollTarget.height() / 2)
                  ),
    
              directionAdjustment = displayContainerAboveInput ?
                  this.$selectionContainer.outerHeight(false) * -1 :
                  this.$input.outerHeight(false),
              
              scrollTargetAdjustment = $scrollTarget.css('position') == 'fixed' ?
                  $scrollTarget.offset().top - $scrollTarget.scrollTop() : 0;
          
          selectionContainerYPos = this.$input.offset().top + directionAdjustment - scrollTargetAdjustment;
          
          this.$container
          .toggleClass('sol-selection-top', displayContainerAboveInput)
          .toggleClass('sol-selection-bottom', !displayContainerAboveInput);
          ...
          this.$selectionContainer.css({
            top: Math.floor(selectionContainerYPos + 0),
      ...
      scrollTargets: undefined,
      maxHeight: undefined,
      converter: undefined,
      asyncBatchSize: 300,
      maxShow: 0,
      displayContainerDirection: 'auto' // top or bottom
    },
    ...
    init: function () {
      ...
      if (!this.config.scrollTarget) {
        this.config.scrollTarget = function () {
         // var selector = window.innerWidth < 992 ? '#filter-container' : '.main-panel';
		    var selector = '.main-panel';
          return $(selector).length ? $(selector) : $(window);
        };
        this.config.scrollTargets = $('.main-panel, #filter-container');
      } else {
        this.config.scrollTargets = this.config.scrollTarget;
      }
    ...
    _initializeUiElements: function () {
      ...
      if (!this.config.maxHeight)
        this.config.maxHeight = this.config.scrollTarget().innerHeight() / 3;
      this.$selection.css('max-height', this.config.maxHeight);
    ...
    _renderOption: function (solOption, $optionalTargetContainer) {
      ...
      $displayElement = $('<div class="sol-option"/>')
      .attr('data-label', solOption.label)
      .append($label);
    ...
    _selectionChange: function ($changeItem, skipCallback) {
      ...
      if (this.config.multiple) {
        ...
        this.config.scrollTargets.trigger('scroll');
      } else {
    ...
    open: function () {
      if (this.isClosed()) {
        ...
        this.config.scrollTargets.on('scroll', this.internalScrollWrapper).trigger('scroll');
    ...
    close: function () {
      if (this.isOpen()) {
        ...
        this.config.scrollTargets.off('scroll', null, this.internalScrollWrapper);
        $(window).off('resize', null, this.internalScrollWrapper);
        ...
        this.config.displayContainerAbove = undefined;
